...
class PerlinFlowField extends Attractor{
    ...
    PerlinFlowField(float noiseScale, float noiseOctaves, boolean scaleNoiseScale){
        this.grid = new PVector[renderWidth][renderHeight];
        float xoff = 0;
        for(int x=0; x<renderWidth; x++){
            float yoff = 0;
...
class Particle {
  ...
  public Particle (PVector p, float ang, float initial_TURN_CHANCES,float initial_TURN_ANGLE,float initial_DEPOSIT_RATE,float initial_DIVISION_CHANCES,float initial_DIVISION_ANGLE,float initial_TERMINATION_THRESHOLD,float initial_TERMINATION_CHANCES, float damp) 
    {
    this.lastPos = p;
    this.pos = p;
    this.ang = ang;
  ...
  void display () {
    ...
    if (line.mag() < 4*STEP_SIZE) {
      ...
      if (idx < renderWidth*renderHeight){grid[idx] += DEPOSIT_RATE;} //update substrate grid when drawing Particle 
    }
  }
